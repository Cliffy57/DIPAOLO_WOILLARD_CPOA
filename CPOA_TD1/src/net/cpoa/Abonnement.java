...
public class Abonnement {
	...
	public static void modifierAbonnement(int IDnew, int IDold, int IDclient, int IDrevue, LocalDate date_debut,
			LocalDate date_fin) {
		try {
			Connection laConnexion = Connexion.creeConnexion();
			PreparedStatement requete = laConnexion.prepareStatement(
	...
	public static void deleteAbonnement(int ID) {
		try {
			...
			PreparedStatement requete = laConnexion
					.prepareStatement("delete from Abonnement where id_abonnement=" + ID);
			int res = requete.executeUpdate();
